...
class ResNet(nn.Module):
    ...
    def _forward_impl(self, x: Tensor) -> Tensor:
        ...
        x = self.conv1(x)  # torch.Size([2, 64, 112, 112])
        x = self.bn1(x)
        x = self.relu(x)
        x = self.maxpool(x)  # torch.Size([2, 64, 56, 56])

        x = self.layer1(x)  # torch.Size([2, 256, 56, 56])
        x = self.layer2(x)  # torch.Size([2, 512, 28, 28])
        x = self.layer3(x)  # torch.Size([2, 1024, 14, 14])
        x = self.layer4(x)  # torch.Size([2, 2048, 7, 7])

        x = self.avgpool(x)  # torch.Size([2, 2048, 1, 1])
        x = torch.flatten(x, 1)
        x = self.fc(x)
