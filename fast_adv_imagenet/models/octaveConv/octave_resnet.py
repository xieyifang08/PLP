...
class OctaveConv(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size, alpha=0.5, stride=1, padding=1, dilation=1,
        ...
        self.conv_1x1_h = torch.nn.Conv2d(int(alpha * out_channels), 1, kernel_size=(1, 1))
        self.conv_1x1_l = torch.nn.Conv2d(int(alpha * out_channels), 1, kernel_size=(1, 1))
    ...
    def forward(self, x):
        ...
        X_h = X_l2h + X_h2h
        X_l = X_h2l + X_l2l
        h = self.conv_1x1_h(X_h)
        l = self.conv_1x1_l(X_l)
        return X_h, X_l, h, l
