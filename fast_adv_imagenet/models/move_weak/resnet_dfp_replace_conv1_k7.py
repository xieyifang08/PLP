...
class DFPModule(nn.Module):
    def __init__(self, in_channels, out_channels, padding, dilation):
        ...
        self.asppConv1 = nn.Sequential(
            nn.Conv2d(in_channels, out_channels // 3, 7, padding=padding[0], stride=2, dilation=dilation[0],
                      bias=False),
            nn.BatchNorm2d(out_channels // 3),
            nn.ReLU()
        )
        self.asppConv2 = nn.Sequential(
            nn.Conv2d(in_channels, out_channels // 3, 7, padding=padding[1], stride=2, dilation=dilation[1],
                      bias=False),
            nn.BatchNorm2d(out_channels // 3),
            nn.ReLU()
        )
        self.asppConv3 = nn.Sequential(
            nn.Conv2d(in_channels, out_channels // 3, 7, padding=padding[2], stride=2, dilation=dilation[2],
                      bias=False),
            nn.BatchNorm2d(out_channels // 3),
            nn.ReLU()
        )
...
class ResNet(nn.Module):
    ...
    def __init__(
        ...
        #                        bias=False)
        self.dfp = DFPModule(3, 64, [3, 9, 15], [1, 3, 5])
