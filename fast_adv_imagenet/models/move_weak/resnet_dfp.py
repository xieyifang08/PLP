...
class DFPModule(nn.Module):
    def __init__(self, in_channels, out_channels, padding, dilation):
        ...
        self.asppConv1 = nn.Sequential(
            nn.Conv2d(in_channels, out_channels, 3, padding=padding[0], stride=2, dilation=dilation[0], bias=False),
            nn.BatchNorm2d(out_channels),
            nn.ReLU()
        )
        self.asppConv2 = nn.Sequential(
            nn.Conv2d(in_channels, out_channels, 3, padding=padding[1], stride=2, dilation=dilation[1], bias=False),
            nn.BatchNorm2d(out_channels),
            nn.ReLU()
        )
        self.asppConv3 = nn.Sequential(
            nn.Conv2d(in_channels, out_channels, 3, padding=padding[2], stride=2, dilation=dilation[2], bias=False),
            nn.BatchNorm2d(out_channels),
            nn.ReLU()
        )
    ...
    def forward(self, x):
        x1 = self.asppConv1(x)  # torch.Size([2, 16, 32, 32])
        x2 = self.asppConv2(x)  # torch.Size([2, 16, 32, 32])
        x3 = self.asppConv3(x)  # torch.Size([2, 16, 32, 32])
        p3 = x3
        p2 = x2 + p3
        p1 = x1 + p2
        p2 = self.smooth2(p2)  # torch.Size([2, 16, 32, 32])
        p1 = self.smooth1(p1)  # torch.Size([2, 16, 32, 32])
        p_out = torch.cat((p1, p2, p3), dim=1)  # torch.Size([2, 48, 32, 32])
        p_out = self.se(p_out)  # torch.Size([2, 48, 32, 32])
        return p_out
...
class ResNet(nn.Module):
    ...
    def __init__(
        ...
        self.dfp = DFPModule(3, 20, [1, 5, 7], [1, 5, 7])
        self.bn1 = norm_layer(self.inplanes)
        self.relu = nn.ReLU(inplace=True)
        self.maxpool = nn.MaxPool2d(kernel_size=3, stride=2, padding=1)
        self.layer1 = self._make_layer(block, 60, layers[0])
