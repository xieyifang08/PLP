...
class ResNet(nn.Module):
    ...
    def forward_non_local_with_label(self, x, labels):
        cams = self.drawCam(x, 304, labels)  # list(batch) tensor(1,8,8)
        # batchs, _, h, w = x.size()
        # for b in range(batchs):
        #     cam = cams[b]
        #     img = cam.cpu().detach().numpy() * 255
        #     # 维度转换（变为64*64*3）
        #     img3 = np.transpose(img, (1, 2, 0))
        #     img3 = img3.astype(np.int32)
        #     plt.figure()
        #     plt.subplot(2, batchs, b+1)  # 将画板分为2行两列，本幅图位于第一个位置
        #     plt.imshow(img3)
        #
        # for b in range(batchs):
        #     img = x[b]
        ...
        #     img3 = img3.astype(np.int32)
        #     plt.figure()
        #     plt.subplot(2, batchs, b+batchs+1)  # 将画板分为2行两列，本幅图位于第一个位置
        #     plt.imshow(img3)
    ...
    def _forward_impl(self, x: Tensor) -> Tensor:
        ...
        assert torch.isnan(x).sum() == 0 and torch.isinf(x).sum() == 0, ('output of input layer is nan or infinit', x.std())  # out 是你本层的输出 out.std()输出标准差
        x = self.conv1(x)
        assert torch.isnan(x).sum() == 0 and torch.isinf(x).sum() == 0, ('output of conv1 layer is nan or infinit', x.std())  # out 是你本层的输出 out.std()输出标准差
        x = self.bn1(x)
        assert torch.isnan(x).sum() == 0 and torch.isinf(x).sum() == 0, ('output of bn1 layer is nan or infinit', x.std())  # out 是你本层的输出 out.std()输出标准差
        x = self.relu(x)
        assert torch.isnan(x).sum() == 0 and torch.isinf(x).sum() == 0, ('output of relu layer is nan or infinit', x.std())  # out 是你本层的输出 out.std()输出标准差
        x = self.maxpool(x)
        assert torch.isnan(x).sum() == 0 and torch.isinf(x).sum() == 0, ('output of maxpool layer is nan or infinit', x.std())  # out 是你本层的输出 out.std()输出标准差
        x = self.layer1(x)
        assert torch.isnan(x).sum() == 0 and torch.isinf(x).sum() == 0, ('output of layer1 layer is nan or infinit', x.std())  # out 是你本层的输出 out.std()输出标准差
        x = self.layer2(x)
        assert torch.isnan(x).sum() == 0 and torch.isinf(x).sum() == 0, ('output of layer2 layer is nan or infinit', x.std())  # out 是你本层的输出 out.std()输出标准差
        x = self.layer3(x)
        assert torch.isnan(x).sum() == 0 and torch.isinf(x).sum() == 0, ('output of layer3 layer is nan or infinit', x.std())  # out 是你本层的输出 out.std()输出标准差
        x = self.layer4(x)
        assert torch.isnan(x).sum() == 0 and torch.isinf(x).sum() == 0, ('output of layer4 layer is nan or infinit', x.std())  # out 是你本层的输出 out.std()输出标准差
        x = self.forward_non_local(x)
        assert torch.isnan(x).sum() == 0 and torch.isinf(x).sum() == 0, ('output of forward_non_local layer is nan or infinit', x.std())  # out 是你本层的输出 out.std()输出标准差
        x = self.avgpool(x)
        assert torch.isnan(x).sum() == 0 and torch.isinf(x).sum() == 0, ('output of avgpool layer is nan or infinit', x.std())  # out 是你本层的输出 out.std()输出标准差
        x = torch.flatten(x, 1)
        assert torch.isnan(x).sum() == 0 and torch.isinf(x).sum() == 0, ('output of flatten layer is nan or infinit', x.std())  # out 是你本层的输出 out.std()输出标准差
        x = self.fc(x)
        assert torch.isnan(x).sum() == 0 and torch.isinf(x).sum() == 0, ('output of fc layer is nan or infinit', x.std())  # out 是你本层的输出 out.std()输出标准差
        return x
...
if __name__ == '__main__':
    ...
    m(torch.rand((1, 3, 224, 224)).to(DEVICE))
    i = 0
