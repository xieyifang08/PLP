...
class WideResNet(nn.Module):
    ...
    def feature_map(self, x):
        ...
        out2 = self.relu(self.bn1(out1))
        #print(out1.size(),out2.size())
        out = F.avg_pool2d(out, 8)
        out = out.view(-1, self.nChannels)
        return out2#self.fc(out)
...
def wide_resnet(pretrained=False, **kwargs):
    model = WideResNet(**kwargs)
    return model
...
if __name__ == '__main__':
    ...
    pred = m.forward(x)
    print("X: ", x.shape, " pred: ", pred.shape)
# X:  torch.Size([128, 3, 32, 32])  pred:  torch.Size([128, 640, 8, 8])