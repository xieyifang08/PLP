...
from fast_adv_imagenet.process.preprocess import _jpeg_compression, _TVM, _gridmask
...
def test_accuracy_with_jpeg(model_name, data_path):
    print("test accuracy with jpeg")
    model = model_utils.load_model(model_name).to(DEVICE)
    model.eval()
    test_loader = load_data_for_defense(os.path.join(args.data, 'dev.csv'), data_path)['dev_data']
    for i, batch_data in enumerate(tqdm.tqdm(test_loader, ncols=80)):
        images, labels = batch_data['image'].to(DEVICE), batch_data['label_idx'].to(DEVICE)
        images = _jpeg_compression(images).to(DEVICE)
        logits = model(images.detach())
        test_accs = AverageMeter()
...
def test_accuracy_with_gridmask(model_name, data_path):
    ...
    for i, batch_data in enumerate(tqdm.tqdm(test_loader, ncols=80)):
        ...
        images = _gridmask(images).to(DEVICE)
        logits = model(images.detach())
        test_accs = AverageMeter()
...
def test_accuracy_with_tvm(model_name, data_path):
    ...
    print('model:{}, data:{}, test accuracy:{} '.format(model_name, data_path, test_accs.avg))
    return test_accs.avg
models = ["vgg19", "resnet50", "resnet152","wide_resnet101_2", "wide_resnet101_2_dnn_jpeg", "ens_adv_inception_resnet_v2"]
datas = ["cw", "fgsm", "high_attack_fgsm_inp_m_2attack", "pgd","pgd_rn50_eps8_step8", "pgd_rn50_eps10_step10","rfgsm"]

ans = []
print("tvm test accuracy")
for model in models:
    for data in datas:
        data_path = '/home/unknown/PLP/fast_adv_imagenet/attacks/advs/'+data
        ans.append("{},{},{}".format(model, data, test_accuracy_with_tvm(model, data_path)))
print(ans)

print("gridmask test accuracy")
ans = []
for model in models:
    for data in datas:
        data_path = '/home/unknown/PLP/fast_adv_imagenet/attacks/advs/'+data
        ans.append("{},{},{}".format(model, data, test_accuracy_with_gridmask(model, data_path)))
print(ans)