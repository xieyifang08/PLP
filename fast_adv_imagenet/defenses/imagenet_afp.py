...
parser.add_argument('--save-folder', '--sf', default='weights/wide_resnet101_imagenet100_afp_noAT/',
                    help='folder to save state dicts')
parser.add_argument('--visdom_env', '--ve', type=str, default="wide_resnet101_imagenet100_afp_noAT",
...
model = model_utils.load_model("wide_resnet101_2_afp", pretrained=False, num_classes=100).to(DEVICE)
...
for epoch in range(args.epochs):
    ...
    for batch_data in tqdm.tqdm(train_loader):
        ...
        if args.adv is not None and epoch >= args.adv:
            ...
            requires_grad_(model, True)
            model.train()
            logits_adv = model(adv.detach(), images)
