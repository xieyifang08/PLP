...
parser.add_argument('--save-folder', '--sf', default='weights/imagenet100_mwe_at/',
                    help='folder to save state dicts')
parser.add_argument('--visdom_env', '--ve', type=str, default="imagenet100_mwe_at+anl_ft")
parser.add_argument('--report_msg', '--rm', type=str, default="imagenet100上的mwe实验,注入anlat权重和dfpat权重，ft几轮")
...
model = model_utils.load_model("imagenet100_wide_resnet101_mwe", pretrained=False, num_classes=100).to(DEVICE)
...
weight_anl = "../defenses/weights/best/best_imagenet100_wrn_anl_at_ep_19_val_acc0.7420.pth"  # imagenet100_wide_resnet101_anl
anl_loaded_state_dict = torch.load(weight_anl)
loaded_state_dict["model.gamma"] = anl_loaded_state_dict["model.gamma"]
loaded_state_dict["model.query_conv.weight"] = anl_loaded_state_dict["model.query_conv.weight"]
loaded_state_dict["model.query_conv.bias"] = anl_loaded_state_dict["model.query_conv.bias"]
loaded_state_dict["model.key_conv.weight"] = anl_loaded_state_dict["model.key_conv.weight"]
loaded_state_dict["model.key_conv.bias"] = anl_loaded_state_dict["model.key_conv.bias"]
loaded_state_dict["model.value_conv.weight"] = anl_loaded_state_dict["model.value_conv.weight"]
loaded_state_dict["model.value_conv.bias"] = anl_loaded_state_dict["model.value_conv.bias"]
...
if args.adv == 0:
    ...
    scheduler = lr_scheduler.MultiStepLR(optimizer, milestones=[20, 50, 100], gamma=0.1)
...
for epoch in range(args.epochs):
    ...
    for batch_data in tqdm.tqdm(train_loader):
        ...
        if epoch < 60:
            ...
            logits = model.forward_with_label(images, labels)
        ...
        if args.adv is not None and epoch >= args.adv:
            ...
            if epoch < 60:
                ...
                logits_adv = model.forward_with_label(adv.detach(), labels)
