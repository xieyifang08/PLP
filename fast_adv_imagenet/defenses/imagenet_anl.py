...
parser.add_argument('--save-folder', '--sf', default='weights/imagenet100_anl_window_threshold/',
                    help='folder to save state dicts')
parser.add_argument('--visdom_env', '--ve', type=str, default="imagenet100_anl_window_threshold")
parser.add_argument('--report_msg', '--rm', type=str, default="imagenet100上的anl模型,window阈值实验")
...
if system == "Windows":
    ...
    train_loader = loader['train_data']
    test_loader = load_data_for_defense("../attacks/advs/alexnet/clean")['dev_data']
...
df_loader = load_data_for_defense("../attacks/advs/alexnet/deepfool_overshoot1")['dev_data']
fgsm_loader = load_data_for_defense("../attacks/advs/alexnet/fgsm")['dev_data']


logging.info(
    "train_loader:{}, val_loader:{}, test_loader:{}, df_loader:{}, fgsm_loader:{}".format(len(train_loader), len(val_loader), len(test_loader), len(df_loader), len(fgsm_loader)))

model = model_utils.load_model("imagenet100_wide_resnet101_anl", pretrained=False, num_classes=100).to(DEVICE)

weight = "../defenses/weights/imagenet100_anl_baseline_add_ft/best_imagenet100_ep_30_val_acc0.7961.pth"  # imagenet100_wide_resnet101_anl
...
for win in range(1, 7 * 2):
    ...
    for epoch in range(args.epochs):
        ...
        with torch.no_grad():
            for batch_data in tqdm.tqdm(df_loader):
                images, labels = batch_data['image'].to(DEVICE), batch_data['label_idx'].to(DEVICE)
                logits = model(images)
                loss = F.cross_entropy(logits, labels)
                val_accs.append((logits.argmax(1) == labels).float().mean().item())
        ...
        with torch.no_grad():
            for batch_data in tqdm.tqdm(fgsm_loader):
                images, labels = batch_data['image'].to(DEVICE), batch_data['label_idx'].to(DEVICE)
                logits = model(images)
                loss = F.cross_entropy(logits, labels)
                val_accs.append((logits.argmax(1) == labels).float().mean().item())
        ...
        if val_accs.avg >= best_acc:  # args.adv is None and
            ...
            files2remove = glob.glob(os.path.join(save_path, "window_%.1f_best_cifar10_*" % window))
            for _i in files2remove:
                os.remove(_i)
            strsave = "window_%.1f_best_imagenet100_ep_%d_val_acc%.4f.pth" % (window, epoch, best_acc)
