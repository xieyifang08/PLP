...
parser.add_argument('--save-folder', '--sf', default='weights/imagenet100_wrn_aad/',
                    help='folder to save state dicts')
parser.add_argument('--visdom_env', '--ve', type=str, default="imagenet100_wrn_aad")
parser.add_argument('--report_msg', '--rm', type=str, default="imagenet100_wrn_baseline， aad实验")
...
model = model_utils.load_model("wide_resnet101_imagenet100_backbone", pretrained=False, num_classes=100).to(DEVICE)
weight = "./weights/best/best_imagenet100_wrn_clean_ep_99_val_acc0.7872.pth"
...
for epoch in range(args.epochs):
    ...
    for batch_data in tqdm.tqdm(train_loader):
        ...
        for j in range(images.shape[0]):
            grayscale_cam = cam(input_tensor=images[j].unsqueeze(0), target_category=labels[j].cpu().item())
            att_list.append(grayscale_cam.copy())
            grayscale_cam[grayscale_cam < 0.5] = 0
            grayscale_cam[grayscale_cam >= 0.5] = 1
            im = images[j].cpu()
            img = im * grayscale_cam
            drop_images.append(img)
        drops = torch.stack(drop_images, dim=0)
        drops_logits = model(drops.to(DEVICE))
        loss_r = F.cross_entropy(drops_logits, labels)

        att_map = torch.from_numpy(np.array(att_list)).to(DEVICE)
        ...
        if args.adv is not None and epoch >= args.adv:
            ...
            loss_c_adv = F.cross_entropy(logits_adv, labels)
            loss_c += loss_c_adv

            drop_images = []
            adv_att_list = []
            for j in range(images.shape[0]):
                grayscale_cam = cam(input_tensor=adv[j].unsqueeze(0), target_category=labels[j].cpu().item())
                adv_att_list.append(grayscale_cam.copy())
                grayscale_cam[grayscale_cam < 0.5] = 0
                grayscale_cam[grayscale_cam >= 0.5] = 1
                im = adv[j].cpu()
                img = im * grayscale_cam
            ...
            drops = torch.stack(drop_images, dim=0)
            drops_logits = model(drops.to(DEVICE))
            loss_r_adv = F.cross_entropy(drops_logits, labels)
            loss_r += loss_r_adv
            adv_att_map = torch.from_numpy(np.array(adv_att_list)).to(DEVICE)
            loss_p = F.mse_loss(att_map, adv_att_map)

        loss = loss_c - 0.01 * loss_r + 0.01*loss_p
        optimizer.zero_grad()
        if args.amp:
