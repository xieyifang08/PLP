...
from fast_adv_imagenet.utils import AverageMeter, save_checkpoint, requires_grad_, NormalizedModel
...
parser.add_argument('--save-folder', '--sf', default='weights/imagenet_baseline/',
...
def load_data_for_defense(input_dir, img_size=args.img_size, batch_size=args.batch_size):
    ...
    # all_imgs = []
    # for i in range(10):
    #     imgs = glob.glob(os.path.join(input_dir, str(i), '*.JPEG'))
    #     all_imgs.extend(imgs[:50])
