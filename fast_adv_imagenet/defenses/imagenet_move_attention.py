...
parser.add_argument('--save-folder', '--sf', default='weights/wide_resnet101_move_AT_finetune_b20_2/',
                    help='folder to save state dicts')
parser.add_argument('--visdom_env', '--ve', type=str, default="wide_resnet101_move_AT_finetune_b20_2",
...
parser.add_argument('--visdom', '--vdm', type=bool, default=True)
...
def prepareData(input_dir, img_size=args.img_size, batch_size=args.batch_size):  # img_size
    ...
    for folder in range(1000):
        ...
        all_imgs.extend(single_imgs[:20])
...
if system == "Windows":
    ...
    loader = prepareData("/mnt/u2/datasets/mini_ILSVRC2012_img_train")
    train_loader = loader['train_data']
    val_loader = loader['val_data']
...
model = model_utils.load_model("wide_resnet101_2_move", pretrained=True).to(DEVICE)
...
for epoch in range(args.epochs):
    ...
    for batch_data in tqdm.tqdm(train_loader):
        ...
        avg_images = F.avg_pool2d(images, 2)
        avg_images_1 = F.avg_pool2d(images, 3)
        for i in range(images.shape[0]):
            ...
            plt.imshow(img)
            plt.subplot(1, 3, 2)
            avg_img = avg_images[i].cpu().numpy()
            avg_img = np.transpose(avg_img, (1, 2, 0)) * 255
            avg_img = avg_img.astype(np.int32)
            plt.imshow(avg_img)
            plt.subplot(1, 3, 3)
            avg_img = avg_images_1[i].cpu().numpy()
            avg_img = np.transpose(avg_img, (1, 2, 0)) * 255
            avg_img = avg_img.astype(np.int32)
            plt.imshow(avg_img)
            plt.show()
        ...
        if args.adv is not None and epoch >= args.adv:
            ...
            requires_grad_(model, True)
            model.train()
            logits_adv, x_adv, x_aim = model.forward_at(adv.detach(), images)

            loss_adv = F.cross_entropy(logits_adv, labels)
            adv_losses.append(loss_adv.item())

            loss = loss + 0.2 *loss_adv + 0.1*F.mse_loss(x_adv, x_aim)
    ...
    logging.info('Epoch {} | Training | Loss: {:.4f}, Accs: {:.4f}'.format(epoch, total_losses.avg, accs.avg))
